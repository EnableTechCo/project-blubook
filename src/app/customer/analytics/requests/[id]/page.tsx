...
import { ArrowLeft, Calendar, Clock, Tag, User, MessageCircle, CheckCircle, Clock as ClockIcon } from "lucide-react";
...
const getStatusConfig = (status: string) => {
  const configs: Record<string, { color: string; icon: JSX.Element; message: string }> = {
    completed: { 
    ...
    triaged: { 
      ...
      icon: <MessageCircle className="w-4 h-4" />,
      message: "Your request has been categorized and assigned to the right team."
    },
    submitted: { 
      color: "bg-amber-500/20 text-amber-300",
      icon: <MessageCircle className="w-4 h-4" />,
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <Card className="p-6 bg-white/5 backdrop-blur-sm border-white/10" title={""}>
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                <Tag className="w-4 h-4 text-slate-400" />
                <div>
                  <p className="text-xs text-slate-400">Category</p>
                  <p className="text-white capitalize">{request.category.replace(/_/g, ' ')}</p>
                </div>
              </div>
              ...
              {request.completed_at && (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-white">{formatDate(request.completed_at)}</p>
                  </div>
                </div>
              )}
            </div>
        ...
        <Card className="p-6 bg-white/5 backdrop-blur-sm border-white/10" title={""}>
          ...
          <p className="text-slate-300 leading-relaxed">
            {request.description || "No additional description provided for this request."}
          </p>
        </Card>
