  ...
  if (ordersQuery.isLoading) {
    return (
      <p className="text-sm text-slate-300">
        Loading admin workflow history...
      </p>
    );
  }
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-200/85">
            Cross-role order history, delivery outcomes, and SLA closure for
            admins and staff.
          </p>
        </div>
              ...
              <div className="space-y-5">
                <div className="flex flex-wrap gap-2">
                  ...
                  {selectedOrder.metadata?.sla_status ? (
                    <Badge>
                      SLA{" "}
                      {selectedOrder.metadata.sla_status === "met"
                        ? "Met"
                        : "Missed"}
                    </Badge>
                  ) : null}
                  {selectedOrder.metadata?.delivered_to ? (
                    <Badge>
                      Delivered to {selectedOrder.metadata.delivered_to}
                    </Badge>
                  ) : null}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  {selectedOrder.metadata?.workflow_timeline?.length ? (
                    <div className="space-y-2">
                      {selectedOrder.metadata.workflow_timeline.map(
                        (event, index) => (
                          <div
